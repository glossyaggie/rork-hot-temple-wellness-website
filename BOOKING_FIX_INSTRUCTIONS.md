## Steps to Fix Your Booking System
...
After running the above SQL, you'll see your user ID. Run this command (replace YOUR_USER_ID with the actual ID):

```sql
INSERT INTO user_passes (user_id, pass_type, remaining_credits, expires_at, is_active) 
VALUES ('YOUR_USER_ID', '10 Class Pass', 10, NOW() + INTERVAL '30 days', true);
```

**3. Test the Booking**

Your app is already set up to use the new RPC functions! The booking should now:
- ✅ Deduct 1 credit when you book a class
- ✅ Show updated balance in "My Passes" card
- ✅ Prevent double-booking
- ✅ Handle capacity limits
- ✅ Only show upcoming classes (past classes are filtered out)

**What's Fixed:**

1. **Credits now deduct properly** - The `book_class` RPC function handles everything atomically
2. **Single source of truth** - All credit data uses `remaining_credits` column
3. **Real-time updates** - MyPassesCard refreshes after booking
4. **Proper filtering** - Only upcoming classes show in your bookings
5. **Error handling** - Clear messages for all booking scenarios

Try booking a class now - your credits should go down by 1!